window.addEventListener('load', function() {
  ...
  function openMenu() {
    ...
    document.getElementById('animated-menu').style.float= 'right';
  }
  ...
  function promFilter() {
    ...
    var prom = select.options[select.selectedIndex].dataset.year; /*The selectedIndex property sets or returns the index of the selected option in a drop-down list.*/
    ...
    var dropoutPercent = ((dropout * 100)/totalStudents).toFixed(1) + '%';
    ...
    } else if (((studentMeetTarget / totalStudents) * 100).toFixed(1) >= 70) {
  ...
  function studentsByGenerations() {
    ...
    var containerOfTeachers = document.getElementById('listOfTeachers');
    function teachersByGenerations() {
      containerOfTeachers.innerHTML = '';
        for (var b = 0; b < arrayStudents.length; b++) {
        arrayStudents[b];
        // Creando un div para almacenar el perfil de la estudiante
        var profile = document.createElement('div');
        profile.classList.add('profile-t');
        containerOfTeachers.appendChild(profile);
        // Creando la imagen para la foto de perfil de la estudiante
        var imgP = document.createElement('img');
        imgP.setAttribute('src', arrayStudents[b].photo);
        imgP.classList.add('img-t');
        profile.appendChild(imgP);
        // Creando contenedor y agregando nombre para estudiante
        var teacherName = document.createElement('p');
        teacherName.textContent = arrayStudents[b].name.toUpperCase();
        teacherName.classList.add('name-teacher');
        profile.appendChild(teacherName);

      }
    }
    teachersByGenerations();
